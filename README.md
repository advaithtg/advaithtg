# 👋 Hi, I'm Advaith T G

🔧 Automation Engineer | PLC Programmer | Industrial Systems

---

## 🚀 About Me

* 🎓 Mechanical & Mechatronics Engineer (Additive Manufacturing)
* 🤖 Junior Automation Engineer with hands-on PLC experience
* ⚙️ Specialized in Industrial Automation 
* 💡 Strong in Structured Text, Ladder Logic & real-time debugging
* 📍 Based in Kerala, India

---

## 💼 Experience

### 🔧 Automation System Engineer — Schnell Pack Systems

📅 Aug 2025 – Present

* Developed PLC software using **B&R platforms** for packaging machines
* Implemented control logic using **Structured Text**
* Performed **online debugging, testing & optimization**
* Supported **commissioning (I/O mapping, fault handling)**
* Designed & modified **HMI screens, alarms & diagnostics**
* Collaborated with electrical & mechanical teams

---

## 🛠️ Technical Skills

### 🔹 PLC Platforms

B&R | Allen-Bradley | Siemens | ABB | Schneider | Omron | Fanuc | Delta

### 🔹 Programming

* Ladder Logic
* Structured Text
* Function Block Diagram (FBD)
* C++

### 🔹 SCADA & HMI

* iFIX | InTouch | WinCC | FactoryTalk View | Citect
* B&R HMI | Schneider HMI

### 🔹 Industrial Systems

* DCS
* Motion Control & Servo Systems
* VFD Installation & Parameterization

### 🔹 Communication Protocols

* Modbus
* EtherNet/IP

### 🔹 Electrical & Instrumentation

* Wiring & Troubleshooting
* Instrument Calibration

---
## 🛠️ Technologies & Tools

### 🔹 PLC Platforms
![B&R](https://img.shields.io/badge/B&R-orange)
![Allen Bradley](https://img.shields.io/badge/Allen--Bradley-red)
![Siemens](https://img.shields.io/badge/Siemens-blue)
![Schneider](https://img.shields.io/badge/Schneider-green)
![ABB](https://img.shields.io/badge/ABB-red)
![Omron](https://img.shields.io/badge/Omron-grey)

---

### 🔹 Programming
![Ladder Logic](https://img.shields.io/badge/Ladder%20Logic-blue)
![Structured Text](https://img.shields.io/badge/Structured%20Text-orange)
![FBD](https://img.shields.io/badge/FBD-lightgrey)
![C++](https://img.shields.io/badge/C++-blue)
![Python](https://img.shields.io/badge/Python-yellow)

---

### 🔹 SCADA & HMI
![WinCC](https://img.shields.io/badge/WinCC-blue)
![FactoryTalk](https://img.shields.io/badge/FactoryTalk-red)
![Intouch](https://img.shields.io/badge/InTouch-purple)
![iFIX](https://img.shields.io/badge/iFIX-green)
![Citect](https://img.shields.io/badge/Citect-orange)

---

### 🔹 Industrial Communication
![Modbus](https://img.shields.io/badge/Modbus-black)
![EtherNet/IP](https://img.shields.io/badge/EtherNet%2FIP-blue)
![Profinet](https://img.shields.io/badge/Profinet-green)

---

### 🔹 Industrial Systems
![DCS](https://img.shields.io/badge/DCS-grey)
![Motion Control](https://img.shields.io/badge/Motion%20Control-blue)
![VFD](https://img.shields.io/badge/VFD-orange)
![Servo Systems](https://img.shields.io/badge/Servo-green)

---

### 🔹 Hardware & Embedded
![Arduino](https://img.shields.io/badge/Arduino-blue)
![Raspberry Pi](https://img.shields.io/badge/Raspberry%20Pi-red)

## 📂 Featured Projects

### ⚡ Automatic Mains Failure (AMF) System

* Developed using **Allen-Bradley PLC**
* Automatic switching between mains & generator
* Implemented safe startup/shutdown logic

---

### 🏭 Silo Simulator

* PLC-based industrial simulation
* Level monitoring, filling & discharge automation
* Focus on process efficiency & safety

---

### 🔩 Automated Metal Sorting Machine

* Sensor-based sorting using PLC
* Integrated inductive & proximity sensors
* Real-time actuation using solenoids

---

### 🤖 Smart Vending Bot with Waste Management ( Final Year Project )

* Built using **Raspberry Pi + Arduino**
* Camera-based object detection
* Automated dispensing + waste segregation

---

### 🦾 Robotic Arm (Arduino)

* Designed using Fusion 360 & ANSYS
* Fabricated with 3D printing
* Tested & optimized using Arduino

---

## 🎓 Certifications

* Post Graduate Diploma in Industrial Automation
* Advanced Training DCS (Honeywell C-300)
* NPTEL – Product Engineering & Design Thinking
* NPTEL – Industry 4.0 & Social Innovation

---

## 🌱 Currently Learning

* Advanced PLC optimization techniques
* Industrial IoT (IIoT)
* Smart manufacturing systems

---

## 🧠 Areas of Interest

* Industrial Automation & Robotics
* Marine Automation
* Product Design & Development

---

## 📫 Contact Me

📧 [advaithtg789@gmail.com](mailto:advaithtg789@gmail.com)
🔗 LinkedIn: https://www.linkedin.com/in/advaith-t-g-T
📱 Phone: +91 9745061137

---

⭐ *“Turning industrial problems into automated solutions.”*

